...
import { Clock, Star, CreditCard, Utensils, ChevronRight, X, ChefHat, Flame, Maximize2, Minimize2 } from 'lucide-react';
...
const HostelMealBooking = () => {
  ...
  // Reusable Content for consistency between Normal and Focused views
  const MealContent = ({ isExpanded }) => (
    <div className={`grid grid-cols-1 ${isExpanded ? 'lg:grid-cols-2 gap-12' : 'lg:grid-cols-2'} relative z-20 h-full`}>
      ...
      <div className="p-6 sm:p-10 flex flex-col justify-center space-y-6 lg:space-y-8">
        ...
        <div className="space-y-3 relative z-30">
          <motion.div
            layoutId="hot-badge"
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-900/30 border border-green-800 text-green-400 text-xs font-bold uppercase tracking-wider shadow-glow-green w-fit"
          >
          ...
          {meals.map((meal, index) => (
            ...
            >
              ...
              <div className="relative w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0 rounded-xl overflow-hidden shadow-lg">
                ...
                />
                {meal.rating >= 4.8 && (
                  <div className="absolute top-0 right-0 bg-yellow-400 text-black text-[9px] font-bold px-1.5 py-0.5 rounded-bl-lg shadow-sm">
                    ★ TOP
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                  <span className="flex items-center gap-1 bg-white/5 px-2 py-0.5 rounded-md"><Clock className="w-3 h-3" /> {meal.time}</span>
                  <span className="flex items-center gap-1 bg-white/5 px-2 py-0.5 rounded-md"><Star className="w-3 h-3 text-yellow-500 fill-current" /> {meal.rating}</span>
                </div>
              </div>
        ...
        {isExpanded ? (
          <div className="grid grid-cols-2 gap-4 p-4 overflow-y-auto h-full custom-scrollbar">
            {[...meals, ...meals, ...meals].map((meal, i) => (
              <div key={`exp-${i}`} className="w-full aspect-square rounded-2xl overflow-hidden relative shadow-2xl border border-white/5 group hover:scale-105 transition-transform duration-300 cursor-pointer" onClick={() => handleBookMeal(meal)}>
